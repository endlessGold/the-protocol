...
async fn main() -> Result<()> {
    ...
    loop {
        ...
        let message = match cmd.as_str() {
            "quit" | "exit" => { println!("Goodbye!"); break; }
            "help" => {
                println!("Commands: look, move <dir>, attack <target>, inventory, create <name> <class>, quit");
                continue;
            }
            "look" => Message::command(Command {
                id: rand::random(), command_type: "look".to_string(), session_id: 0,
                timestamp: chrono::Utc::now().timestamp_millis() as u64, payload: vec![],
            }),
            "move" => {
                let dir = parts.get(1).unwrap_or(&"north");
                let move_cmd = MoveCommand { direction: Direction::from_str(dir).unwrap_or(Direction::North) };
                Message::command(Command {
                    id: rand::random(), command_type: "move".to_string(), session_id: 0,
            ...
            "attack" => {
                ...
                Message::command(Command {
                    id: rand::random(), command_type: "attack".to_string(), session_id: 0,
                    timestamp: chrono::Utc::now().timestamp_millis() as u64,
                    payload: target.as_bytes().to_vec(),
                })
            }
            "inventory" => Message::command(Command {
                id: rand::random(), command_type: "inventory".to_string(), session_id: 0,
                timestamp: chrono::Utc::now().timestamp_millis() as u64, payload: vec![],
            }),
            "create" => {
                ...
                Message::command(Command {
                    id: rand::random(), command_type: "create_character".to_string(), session_id: 0,
                    timestamp: chrono::Utc::now().timestamp_millis() as u64,
                ...
                if let Some(response) = ProtocolCodec::decode_simple(&mut buf)? {
                    match response.message_type {
                        MessageType::CommandResponse => {
                            ...
                            if resp.success {
                                if let Ok(look) = rmp_serde::from_slice::<LookResponse>(&resp.payload) {
                                    println!("\n=== {} ===", look.room_name);
                                    println!("{}", look.room_description);
                                    if !look.exits.is_empty() { println!("\nExits: {}", look.exits.join(", ")); }
                                    for p in &look.players { println!("{} is here.", p.name); }
                                    for n in &look.npcs { println!("{} (HP: {}/{})", n.name, n.hp, n.max_hp); }
                                } else if let Ok(mv) = rmp_serde::from_slice::<MoveResponse>(&resp.payload) {
                                    println!("Moved to {}.", mv.room_name.unwrap_or_default());
                                    println!("{}", mv.room_description.unwrap_or_default());
                                } else if let Ok(atk) = rmp_serde::from_slice::<AttackResponse>(&resp.payload) {
                                    println!("{}", atk.message.unwrap_or_default());
                                } else if let Ok(inv) = rmp_serde::from_slice::<InventoryResponse>(&resp.payload) {
                                    println!("\n=== Inventory ===");
                                    for item in &inv.items { println!("  {} x{}", item.name, item.quantity); }
                                    println!("Gold: {}", inv.gold);
                                } else { println!("Response received."); }
                            } else { println!("Error: {}", resp.error.unwrap_or_default()); }
                        }
                        MessageType::Error => {
